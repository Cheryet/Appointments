...
test("getInterviewersForDay returns an array containing the correct interviewer objects", () => {
  ...
  expect(first).toEqual(state.interviews["3"]);
  expect(second).toEqual(state.interviews["2"]);
});

test("getInterviewersForDay returns an array containing the correct interviewer objects", () => {
  const [first, second, third] = getInterviewersForDay(state, "Monday");
  expect(first).toEqual(state.interviews["1"]);
  expect(second).toEqual(state.interviews["2"]);
  expect(third).toEqual(state.interviews["4"]);
});
